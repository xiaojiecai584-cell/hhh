...
import { solveGroundContact, ZERO_ANGLES } from '../core/motion/groundContact'
...
export default function HumanViewport({
  ...
  useEffect(() => {
    ...
    // 初始定位（零角度），用于相机取景
    const initial = solveGroundContact(posture, seg, ZERO_ANGLES)
    humanoid.group.rotation.copy(initial.rotation)
    humanoid.group.position.copy(initial.position)
    ...
    const animate = () => {
      ...
      if (pose) {
        ...
        const contact = solveGroundContact(posture, seg, pose)
        humanoid.group.rotation.copy(contact.rotation)
        humanoid.group.position.copy(contact.position)
      }
      controls.update()
